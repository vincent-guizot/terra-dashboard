...
export function SectionContainer({ title, description, actions, className, children, ...props }: SectionContainerProps) {
  return (
    <section
      className={cn("rounded-lg border border-border-default bg-surface px-6 py-5", className)}
      {...props}
    >
      {(title || actions) && (
